    ...
    <div class="p-6">
        <div class=" md:block mb-6">
            <div class="md:h-10 xl:h-16 flex justify-center md:justify-start">
                <img src="/images/logo-vds-2025.png" alt="Logo Vem Dançar Sudamérica" width="900" height="156" class="max-h-10 xl:max-h-10 w-auto">
            </div>
        </div> 

        <h1 class="text-white text-sm font-bold mb-4 text-center md:text-start md:block mb-6 xl:mb-8">Inscrição VDS 2025</h1>
        ...
        <div class="flex flex-row justify-around md:flex-col space-y-1 xl:space-y-8 text-white">
            @foreach($this->steps as $stepNumber => $stepLabel)
                ...
                <div class="flex items-center space-x-0 md:space-x-3 {{ $currentStep == $stepNumber ? 'md:bg-secondary-700' : '' }} hover:bg-secondary-700 p-0 md:p-2 rounded-lg hover:cursor-pointer " data-step="{{ $stepNumber }}" wire:click="goToStep({{ $stepNumber }})">
                    <div class="w-8 h-8 {{ $currentStep >= $stepNumber ? 'bg-primary-600' : 'bg-secondary-500' }} rounded-full flex items-center justify-center text-sm font-semibold border-2 border-transparent {{ $isFinished && $stepNumber < $totalSteps ? 'opacity-50' : 'opacity-100' }}">{{ $stepNumber }}</div>
