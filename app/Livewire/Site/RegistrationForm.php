...
class RegistrationForm extends Component
{
    ...
    public $registrationsOpenToPublic = true;

    public $canEditAfterSubmission = false;
    ...
    {
        if ($this->registration?->id && $this->choreographies->isNotEmpty()) {
            return 6;
        }
        if ($this->registration?->id && $this->dancers->isNotEmpty()) {
            return 5;
        }
        if ($this->registration?->id && $this->members->isNotEmpty()) {
            return 4;
        }
        if ($this->registration?->id && $this->school->exists) {
            return 3;
        }
        if ($this->school->exists) {
            return 2;
